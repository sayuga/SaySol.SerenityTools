...
Default to the `native/serenity` catalog and the dependency-free SaySol builders.
Install an `external/<provider>` adapter only when the user explicitly selects
it and the provider manifest declares its exact package/assets, license,
security review, and validation evidence.
